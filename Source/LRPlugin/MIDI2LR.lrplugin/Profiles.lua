...
local dialogorder = {'Tool','Module','Panel',''}
...
local function changeProfile(profilename, ignoreCurrent)
  if type(ProgramPreferences.Profiles[profilename]) == 'table' then
      useDefaults() --protect against improper initialization of preferences in beta version--remove this in final
    end
    local changed = false
...
--repeat for each type and ''
    --f:group_box{title = profileType} -- then add f:column that contains:
--this is still a mess
    for i,profileType in ipairs(dialogorder) do
      local groupbox = f:group_box{title = profileType}
      for k,v in pairs(ProfileTypes) do
        if v['ToolModulePanel'] == profileType then
          table.insert(groupbox,
            f:row { 
              f:static_text{title = v['friendlyName'],width = LrView.share('profile_label')},
              f:edit_field{value = LrView.bind('Profile'..k),width = LrView.share('profile_text')}
            })
        end
      end
      table.insert(allboxes,groupbox)
    end
    return unpack(allboxes)
  end
