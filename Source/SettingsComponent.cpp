...
void SettingsComponent::Init()
{
   ...
   pickup_label_.setText("Disabling the pickup mode may be better for touchscreen interfaces and "
   ...
   autohide_explain_label_.setText("Autohide the plugin window in x seconds, select 0 for "
                                   "disabling autohide",
...
{ //-V2009 overridden method
   if (button == &pickup_enabled_) {
      ...
      juce::FileChooserDialogBox dialog_box{"Select Profile Folder",
          "Select a folder containing MIDI2LR Profiles", browser, true, juce::Colours::lightgrey};
