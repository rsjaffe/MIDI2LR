...
# define gsl_CONFIG_THROWS_FOR_TESTING  NDEBUG
#endif
...
#define Ensures(x)  ::gsl::fail_fast_assert((x))
  ...
  class not_null {
  public:
    gsl_api not_null(T t): ptr_(t) {
      Expects(ptr_ != NULL);
    }
    gsl_api not_null & operator=(T const & t) {
      ptr_ = t;  Expects(ptr_ != NULL); return *this;
    }
  ...
  public:
    ...
    gsl_api      operator T() const {
      return get();
    }
    gsl_api T    operator->() const {
      return get();
    }

    gsl_api bool operator==(T const & rhs) const {
      return    ptr_ == rhs;
    }
    gsl_api bool operator!=(T const & rhs) const {
      return !(*this == rhs);
